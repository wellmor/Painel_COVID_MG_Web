...
function App() {
	...
	const [mapViewport, setMapViewport] = useState({ center: [-21.274818462189064, -43.17910194396973], zoom: 12 });
	...
	const onSelectLocation = useCallback((id) => {
		...
		api.getLocation(id).then(response => {
			...
			if (appRef.current.offsetWidth < mqDesktop) {
				if (latitude >= -21.2335) nextLatitude -= 0.5
				else if (latitude < -21.2335 && latitude >= -43.1728) nextLatitude -= 1;
				else if (latitude < -21.2335 && latitude >= -43.1728) nextLatitude -= 1.5;
				else nextLatitude -= 2;
			}

			setSelectedLocation(location);
			setMapViewport({ center: [nextLatitude, longitude], zoom: 15 });
		}).catch(error => {
			console.error(error);
			setSelectedLocation(null);
			setError(error);
